...
volatile uint8_t flg_flrx=0;
volatile uint8_t iflrxcount=0;
volatile uint8_t iflrxcount2=0;
volatile uint8_t iflrxdata[10]={0};
volatile uint8_t iflrxdata2[10]={0};
...
*/
status_t FLEXIO_UART_Init(FLEXIO_UART_Type *base, const flexio_uart_config_t *userConfig, uint32_t srcClock_Hz)   //org
{
...
status_t FLEXIO_UART_Init_tx(FLEXIO_UART_Type *base, const flexio_uart_config_t *userConfig, uint32_t srcClock_Hz)  
{
    assert((base != NULL) && (userConfig != NULL));
...
status_t FLEXIO_UART_Init_rx(FLEXIO_UART_Type *base, const flexio_uart_config_t *userConfig, uint32_t srcClock_Hz)  
{
    assert((base != NULL) && (userConfig != NULL));
...
void FLEXIO_UART_Deinit_tx(FLEXIO_UART_Type *base)
{
    base->flexioBase->SHIFTCFG[base->shifterIndex[0]] = 0;
    base->flexioBase->SHIFTCTL[base->shifterIndex[0]] = 0;
...
void FLEXIO_UART_Deinit_rx(FLEXIO_UART_Type *base)
{
 ...
 */
void FLEXIO_UART_TransferHandleIRQ(void *uartType, void *uartHandle)  //chg
{   
    uint8_t count                = 1;
...
//PRINTF("rx in irq\r\n",status); 
            ...
            uint8_t cbyte= (uint8_t)(base->flexioBase->SHIFTBUFBYS[base->shifterIndex[1]]);  

            iflrxdata[iflrxcount++]=cbyte;
            if( iflrxcount>=5)
            {
                  flg_flrx=1; 
                  iflrxcount2=iflrxcount;
                  memcpy((void *)iflrxdata2,(const void *)iflrxdata,iflrxcount);
...
//PRINTF("tx outed %d\r\n",status);      
            ...
            if (0U == handle->txDataSize)
            {
                ...
                FLEXIO_UART_DisableInterrupts(base, (uint32_t)kFLEXIO_UART_TxDataRegEmptyInterruptEnable);
                //FLEXIO_UART_ClearStatusFlags(base, (uint32_t)kFLEXIO_UART_TxDataRegEmptyFlag);  //--add
...
////test 
////FLEXIO_UART_ClearStatusFlags(base, (uint32_t)kFLEXIO_UART_TxDataRegEmptyFlag);      
//status = FLEXIO_UART_GetStatusFlags(base);    
//PRINTF("out irq %d\r\n",status);  
