...
#include "fsl_debug_console.h"  //--add
#include "board.h"  //--add

uint8_t flg_sai_rx=0;
uint8_t flg_sai_tx=0;
uint8_t flg_sai3_rx=0;

volatile int32_t iaudioL=0;
volatile int32_t iaudioR=0;
volatile int32_t iaudioL_detect=0;
volatile int32_t iaudioR_detect=0;
volatile uint8_t flg_detect=0;
...
static sai_rx_isr_t s_saiRxIsr3;  //--sai3
...
static void SAI_WriteNonBlocking(I2S_Type *base,        //--org
                                 uint32_t channel,
                                 uint32_t channelMask,
                                 uint32_t endChannel,
                                 uint8_t bitWidth,
...
////static void SAI_WriteNonBlocking(I2S_Type *base,        //--double tx0
////                                 uint32_t channel,
////                                 uint32_t channelMask,
////                                 uint32_t endChannel,
////                                 uint8_t bitWidth,
...
////static void SAI_WriteNonBlocking(I2S_Type *base,                 //txdata01 use
////                                 uint32_t channel,
////                                 uint32_t channelMask,
////                                 uint32_t endChannel,
////                                 uint8_t bitWidth,
...
////static void SAI_ReadNonBlocking(I2S_Type *base,         //org
////                                uint32_t channel,
////                                uint32_t channelMask,
////                                uint32_t endChannel,
////                                uint8_t bitWidth,
...
static void SAI_ReadNonBlocking(I2S_Type *base,         //isum
                                uint32_t channel,
                                uint32_t channelMask,
                                uint32_t endChannel,
                                uint8_t bitWidth,
        ...
        for (j = channel; j <= endChannel; j++)
        {
            if (IS_SAI_FLAG_SET((1UL << j), channelMask))
            {
                ...
                //if(flg_detect==1)
                {
                   ...
                   {
                        //iaudioL=  data ;
                        iaudioL= (int32_t)(data);
                        iaudioL_detect+= (iaudioL>0?iaudioL:-iaudioL) >>(size/bytesPerWord/2);  //16
                   }
                   else
                   {
                        //iaudioR=data;
                        iaudioR=(int32_t)(data);
                        iaudioR_detect+=(iaudioR>0?iaudioR:-iaudioR)>>(size/bytesPerWord/2);  //16
                   }    
                }                
...
////static void SAI_ReadNonBlocking(I2S_Type *base,         //  1/2
////                                uint32_t channel,
////                                uint32_t channelMask,
////                                uint32_t endChannel,
////                                uint8_t bitWidth,
...
////            {
////                data = base->RDR[j];
////                //data = (uint32_t)(data>>2) & 0xffffffffU;   //--add
////               
////                for (m = 0; m < bytesPerWord; m++)
////                {
////                    *buffer = (uint8_t)(data >> (8U * m)) & 0xFFU;
...
////static void SAI_ReadNonBlocking(I2S_Type *base,   //use1
////                                uint32_t channel,
////                                uint32_t channelMask,
////                                uint32_t endChannel,
////                                uint8_t bitWidth,
...
//static void SAI_ReadNonBlocking(I2S_Type *base,      //use2
//                                uint32_t channel,
//                                uint32_t channelMask,
//                                uint32_t endChannel,
//                                uint8_t bitWidth,
...
void SAI_TransferRxCreateHandle(I2S_Type *base, sai_handle_t *handle, sai_transfer_callback_t callback, void *userData)
{
    ...
    s_saiRxIsr3 = SAI3_TransferRxHandleIRQ;
...
void SAI3_TransferRxHandleIRQ(I2S_Type *base, sai_handle_t *handle)
{
    assert(handle != NULL);
...
#endif
    {
        s_saiRxIsr(SAI1, s_saiHandle[1][1]);   //rx
    }
#if defined(FSL_FEATURE_SAI_FIFO_COUNT) && (FSL_FEATURE_SAI_FIFO_COUNT > 1)
...
#endif
    {
        s_saiTxIsr(SAI1, s_saiHandle[1][0]);  //tx
        flg_sai_tx=1; 
    }
    SDK_ISR_EXIT_BARRIER;
}
