...
void outstandby(void)
{
   ...
   CLOCK_InitAudioPll(&audioPllConfig);
      ...
      //i2s out standby
      set_SAI1_pins_outstandby();      
      set_SAI3_pins_outstandby();      
...
int main(void)      //WORK BOARD
{
   ...
   lpuart_config_t config1;   
   LPUART_GetDefaultConfig(&config1);
   config1.baudRate_Bps = BOARD_DEBUG_UART_BAUDRATE;
   config1.enableTx     = true;
   config1.enableRx     = true;

   LPUART_Init(LPUART1, &config1, DEMO_LPUART_CLK_FREQ);
   ...
   EnableIRQ(DEMO_LPUART_IRQn);
      ...
      GPIO_PinInit(GPIO2, 1, &output_config1); 
      //GPIO_SetPinsOutput(GPIO2, 1 << 1);       //set 1, open LED_UI_WHITE  (before is timeline )
      GPIO_ClearPinsOutput(GPIO2, 1 << 1);       //close
   ...
   //i2s init
      ...
      #if (defined BTMUSIC) 
      SAI3_init();        
      NVIC_SetPriority(SAI3_RX_IRQn,2);      
...
//--------------flex uart init-----------------
   ...
   flexuart_init(BOARD_DEBUG_UART_BAUDRATE);  

   //--tx  
   PRINTF("tx...\r\n");   //--add  
   for (uint32_t i = 0; i < (1000*1000000); i++)    
     __NOP();    

   //FLEXIO_UART_Deinit_rx(&uartDev); 
   FLEXIO_UART_Init_tx(&uartDev, &config, FLEXIO_CLOCK_FREQUENCY);   
    
   uart_send_string(&uartDev,(uint8_t*)"aaa\r\n",7);
  

   //--rx     
   PRINTF("rx...\r\n");   //--add  
   //FLEXIO_UART_Deinit_tx(&uartDev);
   FLEXIO_UART_Init_rx(&uartDev, &config, FLEXIO_CLOCK_FREQUENCY); 
   FLEXIO_UART_EnableInterrupts(&uartDev, (uint32_t)kFLEXIO_UART_RxDataRegFullInterruptEnable);      
    ...
    while (1)
    {
         ...
         if(flg_flrx==0)
         {
            for (uint32_t i = 0; i < (500*1000000); i++)    
               __NOP();   
            ...
            //FLEXIO_UART_Deinit_rx(&uartDev); 
            FLEXIO_UART_Init_tx(&uartDev, &config, FLEXIO_CLOCK_FREQUENCY);               
            uart_send_string(&uartDev,(uint8_t*)"2",1);   
         }       
         ...
         if(flg_flrx==1)
         {
            ...
            //--tx  
            //LPUART_WriteBlocking(LPUART1,(uint8_t*)iflrxdata2,5);              
            LPUART_WriteBlocking(LPUART1,(uint8_t*)iflrxdata2,5);              
            PRINTF( "abcde\r\n" );  
            //LPUART_WriteBlocking(LPUART1,(uint8_t*)"rrrrr\r\n" ,7);              
            //PRINTF( "rrrrr\r\n" );  
    ...
    while (1)
    {
       ...
       if(flg_flrx==1)
       {
            ...
            uart_send_string(&uartDev,(uint8_t*)iflrxdata2,5);          
            //uart_send_string(&uartDev,(uint8_t*)"abcdf\r\n",7);           
 
            //for (uint32_t i = 0; i < (100*1000000); i++)     __NOP();   
    ...
    while (1)
    {
       ...
       if(flg_flrx==1)
       {
            ...
            for (uint32_t i = 0; i < (2000*1000000); i++)    
              __NOP();    
            for (uint32_t i = 0; i < (1000*1000000); i++)    
              __NOP();    
   ...
   while (1)
   {
      ...
      static uint32_t isoundL=0;
      static uint32_t isoundR=0;       
      ...
      //if(imaincount++>50000000)  //1500ms
      {
         ...
         {   
            //if(flg_detect==2)  
            {
               ...
               iaudioL_detect=iaudioL_detect>>6;                  
               if(iaudioL_detect>SOUNDLEVEL)
                  isoundL++;
               
               iaudioR_detect=iaudioR_detect>>6;
               if(iaudioR_detect>SOUNDLEVEL)
                  isoundR++;
               
               //PRINTF("LR: %d %d\r\n ",iaudioL_detect>>10,iaudioR_detect>>10);     
               PRINTF("LR: %d %d %d %d\r\n ",iaudioL_detect,iaudioR_detect,isoundL,isoundR );     
            }
         ...
         //audio wakeup
         if((isoundL>=SOUNDNUM) || (isoundR>=SOUNDNUM) )
         {
            isoundL=0;
            isoundR=0;
